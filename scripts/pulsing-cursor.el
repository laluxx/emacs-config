...
(defun update-pulsing-cursor-color ()
  "Update the pulsing cursor color to match the current cursor color."
  (interactive)
  (let ((cursor-color (face-attribute 'cursor :background nil 'default)))
    (set-face-attribute 'pulsing-cursor-overlay-face1 nil :background cursor-color)))

;; (add-hook 'emacs-startup-hook 'update-pulsing-cursor-color)
